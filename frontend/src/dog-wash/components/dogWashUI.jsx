...
// Secondary Button (Cream background, gray border)
...
export function FormInput({ label, name, type = "text", register, errors, placeholder = "", required = false, ...props }) {
  return (
    <div className="flex flex-col gap-1.5 w-full text-zinc-850">
      {label && (
        <label className="text-xs md:text-sm font-semibold text-zinc-500 ml-1">
          {label} {required && <span className="text-grooming-primary">*</span>}
        </label>
      )}
      <input
        type={type}
        ...
        className={`w-full py-3.5 px-5 bg-white border ${
          errors?.[name] ? 'border-red-500' : 'border-zinc-200 focus:border-grooming-primary'
  ...
  return (
    <div className="flex flex-col gap-1.5 w-full text-zinc-850">
      ...
      <div className="relative">
        <select
          {...(register ? register(name, { required }) : {})}
          className="w-full py-3.5 px-5 bg-white border border-zinc-200 text-zinc-800 rounded-20 outline-none focus:border-grooming-primary appearance-none cursor-pointer transition-all shadow-sm"
          style={{
...
export function DatePicker({ value, onChange }) {
  ...
  return (
    <div className="w-full">
      <div className="flex items-center gap-2 mb-3 text-zinc-500 text-xs md:text-sm font-semibold ml-1">
            ...
            <motion.button
              ...
              className={`flex-shrink-0 flex flex-col items-center justify-center w-16 h-20 rounded-20 border transition-all ${
                isSelected
                  ...
                  : 'bg-white border-zinc-200 text-zinc-650 hover:bg-grooming-cream hover:text-zinc-800 shadow-sm'
              }`}
            >
      ...
      {isOpen && (
        ...
        >
          <div className="flex items-center gap-3 py-3.5 px-5 bg-white border border-zinc-200 rounded-20 shadow-premium backdrop-blur-md max-w-sm w-full text-zinc-800">
            {type === "success" && <CheckCircle className="w-5 h-5 text-grooming-primary flex-shrink-0" />}
            ...
            <span className="text-zinc-750 text-xs md:text-sm font-semibold mr-2 flex-grow">{message}</span>

            <button onClick={onClose} className="text-zinc-450 hover:text-zinc-750 transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
        </motion.div>
...
export function SkeletonLoader({ type = "card", count = 1 }) {
  ...
  if (type === "card") {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
        {items.map((_, i) => (
          <div key={i} className="animate-pulse bg-white border border-zinc-200 rounded-24 p-6 h-64 flex flex-col justify-between shadow-sm">
  ...
  if (type === "banner") {
    return (
      <div className="animate-pulse bg-zinc-100 border border-zinc-250 rounded-24 w-full h-[400px]" />
    );
  }
