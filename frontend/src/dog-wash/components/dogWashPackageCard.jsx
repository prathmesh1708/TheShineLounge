...
export default function DogWashPackageCard({ pack }) {
  ...
  return (
    ...
    >
      {/* Popular badge */}
      {pack.popular && (
        <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-grooming-primary text-white text-[9px] sm:text-[10px] uppercase tracking-widest font-extrabold px-3 py-1 sm:px-3.5 sm:py-1.5 rounded-full shadow-premium border border-white/10 whitespace-nowrap">
          {pack.badge}
        </span>
      )}
      ...
      <div className="space-y-3 sm:space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-1 sm:gap-2">
          ...
          <div className="flex flex-row sm:flex-col items-baseline sm:items-end gap-1.5 sm:gap-0.5 mt-1 sm:mt-0">
            <span className="text-base sm:text-xl md:text-2xl font-extrabold text-zinc-850">${pack.price}</span>
          ...
          <ul className="space-y-2 sm:space-y-3">
            {pack.features.slice(0, 4).map((feature, idx) => (
              <li key={idx} className="flex gap-2 sm:gap-3 items-start text-[10px] sm:text-xs md:text-sm text-zinc-650 font-medium">
                <Check className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-grooming-primary flex-shrink-0 mt-0.5" />
