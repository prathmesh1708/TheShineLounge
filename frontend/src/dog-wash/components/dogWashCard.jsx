  ...
  return (
    ...
    >
      ...
      <div className="relative h-28 sm:h-36 md:h-48 w-full overflow-hidden">
        ...
        />
        <div className="absolute top-2 left-2 sm:top-4 sm:left-4 py-1 px-2 sm:py-1.5 sm:px-3 bg-zinc-900/90 backdrop-blur-md text-[9px] sm:text-xs font-bold text-grooming-primary border border-white/10 rounded-full shadow-sm">
          {service.category}
        </div>
      </div>
      ...
      <div className="p-3 sm:p-5 flex-grow flex flex-col justify-between">
        <div className="space-y-1.5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1">
            <h3 className="text-xs sm:text-sm md:text-lg font-bold text-zinc-800 group-hover:text-grooming-primary transition-colors leading-snug truncate">
              {service.name}
            </h3>
        ...
        <div className="mt-3 pt-2.5 sm:mt-6 sm:pt-4 border-t border-zinc-100 flex items-center justify-between gap-2">
          ...
          <button
            onClick={handleBook}
            className="py-1.5 px-2.5 sm:py-2.5 sm:px-3 bg-grooming-primary hover:bg-grooming-hover text-white rounded-15 sm:rounded-20 shadow-premium transition-all flex items-center justify-center gap-1 text-[9px] sm:text-xs font-bold"
          >
            <span>Book</span>
