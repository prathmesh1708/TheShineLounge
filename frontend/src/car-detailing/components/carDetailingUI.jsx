...
export function SecondaryButton({ children, onClick, type = "button", disabled = false, className = "" }) {
  return (
    <motion.button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`w-full py-2.5 sm:py-3.5 px-4 sm:px-6 bg-white hover:bg-zinc-50 border border-zinc-200 text-zinc-800 font-semibold rounded-16 sm:rounded-20 md:rounded-24 transition-all shadow-sm flex items-center justify-center gap-2 text-xs sm:text-sm md:text-base ${className}`}
    >
      {children}
    </motion.button>
  );
