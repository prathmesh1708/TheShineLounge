...
export default function CarDetailingBookingCard({ booking = {} }) {
  ...
  const statusColors = {
    ...
    Completed: "bg-[#FF6B00]/10 text-[#FF6B00] border-[#FF6B00]/25",
    ...
    <div className="bg-white border border-zinc-200/85 rounded-24 p-6 relative flex flex-col justify-between gap-6 shadow-premium text-zinc-800">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 border-t border-b border-zinc-100 py-4 text-xs md:text-sm text-zinc-600">
        <div className="space-y-2.5">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-semibold text-zinc-800">{booking?.vehicle || 'Tesla Model 3'}</span>
          </div>
        </div>
        ...
        <div className="space-y-2.5">
          ...
          <div className="flex items-center gap-2 pt-0.5">
            ...
            <span className="text-base font-extrabold text-zinc-800">₹{(booking?.price || 149) * 10}</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3.5 pt-1">
        ...
        {currentStatus === "Upcoming" && (
          <button
            onClick={handleTrack}
            className="flex items-center gap-1.5 px-4 py-2.5 bg-[#FF6B00] hover:bg-[#E66000] text-white text-xs font-bold rounded-20 shadow-premium transition-all"
          >
