  ...
  return (
    <motion.div
      ...
      className={`relative flex flex-col justify-between p-6 md:p-8 rounded-24 border ${
        pack.popular
          ? 'bg-gradient-to-b from-luxury-emerald/5 to-white border-luxury-emerald shadow-premium ring-1 ring-luxury-emerald/20'
          : 'bg-white border-zinc-200/85 hover:border-zinc-300 shadow-sm'
      } text-zinc-800 h-full`}
    >
      {/* Popular Badge */}
      {pack.popular && (
        <span className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-luxury-emerald text-white text-[10px] uppercase tracking-widest font-extrabold px-3.5 py-1.5 rounded-full shadow-premium border border-white/10">
          {pack.badge}
        </span>
      )}
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-start">
          ...
          <div className="flex flex-col items-end">
            <span className="text-xl md:text-2xl font-extrabold text-zinc-850">₹{pack.price * 10}</span>
        ...
        <div className="border-t border-zinc-100 pt-6">
          <ul className="space-y-3">
            {pack.features.map((feature, idx) => (
              <li key={idx} className="flex gap-3 items-start text-xs md:text-sm text-zinc-650">
                <Check className="w-4 h-4 text-luxury-emerald flex-shrink-0 mt-0.5" />
                <span>{feature}</span>
              </li>
            ))}
          </ul>
