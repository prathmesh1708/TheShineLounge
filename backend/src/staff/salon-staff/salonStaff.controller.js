const salonStaffService = require('./salonStaff.service');
const { formatResponse } = require('../../common/utils/helpers');

const createStaff = async (req, res, next) => {
  try {
    const newStaff = await salonStaffService.addStaff(req.body);
...
const getStaff = async (req, res, next) => {
  try {
    const staff = await salonStaffService.fetchStaffById(req.params.id);
    if (!staff) {
...
const updateStaff = async (req, res, next) => {
  try {
    const updatedStaff = await salonStaffService.modifyStaff(req.params.id, req.body);
    if (!updatedStaff) {
      return res.status(404).json(formatResponse(false, 'Staff member not found'));
    }
...
const deleteStaff = async (req, res, next) => {
  try {
    const deleted = await salonStaffService.removeStaff(req.params.id);
    if (!deleted) {
      return res.status(404).json(formatResponse(false, 'Staff member not found'));
    }
...
const listStaff = async (req, res, next) => {
  try {
    const staffList = await salonStaffService.fetchStaffList();
